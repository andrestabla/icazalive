...
export default function RoomClient({
  ...
  useEffect(() => {
    ...
    const applyStreamPayload = (raw: string) => {
      ...
      let payload: { type?: string; messages?: RoomData["messages"]; reactions?: RoomData["reactions"]; modules?: RoomData["modules"] };
      try {
      ...
      if (payload.type === "modules" && payload.modules) {
  ...
  return (
    <main className={`participant-room branded-room${enabledPanels.length ? "" : " no-interaction"}`} style={brandStyle}>
      ...
      <div className="room-body">
        <section className="room-main">
          ...
          <div className="room-session-info">
            ...
            <div><span>⌁</span><p><small>ESTADO TÉCNICO</small><b>{room.session.streamingStatus === "ready" ? "Lista" : "En preparación"}</b></p></div>
        ...
        {enabledPanels.length > 0 && (
          <aside className="room-interaction">
            <nav aria-label="Paneles de interacción">
              {roomModules.chat && <button className={visiblePanel === "chat" ? "active" : ""} onClick={() => setActivePanel("chat")}>Chat <span>{room.messages.length}</span></button>}
              {roomModules.questions && <button className={visiblePanel === "questions" ? "active" : ""} onClick={() => setActivePanel("questions")}>Preguntas <span>{room.questions.length}</span></button>}
              {roomModules.polls && <button className={visiblePanel === "polls" ? "active" : ""} onClick={() => setActivePanel("polls")}>Encuestas <span>{room.polls.filter((poll) => poll.status === "open").length}</span></button>}
              {roomModules.resources && <button className={visiblePanel === "resources" ? "active" : ""} onClick={() => setActivePanel("resources")}>Recursos <span>{room.resources.length}</span></button>}
            </nav>
