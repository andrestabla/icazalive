...
export default function ExtraIntegrationCards() {
  ...
  const refresh = () => {
    fetch("/api/email-settings", { cache: "no-store" })
      ...
      .then((p: { data?: { enabled: boolean; provider?: string } } | null) => {
        setSmtpEnabled(Boolean(p?.data?.enabled));
        setEmailProvider(p?.data?.provider === "sendgrid" ? "sendgrid" : "smtp");
      })
  ...
  return (
    <>
      <article className="panel setup-wizard-card smtp-wizard-card">
        <header>
          <span className={`service-logo ${emailProvider === "sendgrid" ? "sendgrid" : "smtp"}`}>
            {emailProvider === "sendgrid" ? "SG" : "SMTP"}
          </span>
          {badge(smtpEnabled)}
        </header>
        <p className="eyebrow">CORREO SALIENTE</p>
        <h3>{emailProvider === "sendgrid" ? "SendGrid" : "Servidor SMTP"}</h3>
        <p>
          Elige entre servidor SMTP o SendGrid y configúralo desde aquí. Con
          prioridad sobre las variables del servidor.
        </p>
        <div className="wizard-card-progress">
          <span style={{ width: smtpEnabled ? "100%" : "0%" }} />
        </div>
        <footer>
          <small>{smtpEnabled ? `Envío activo · ${emailProvider === "sendgrid" ? "SendGrid" : "SMTP"}` : "Sin configurar"}</small>
