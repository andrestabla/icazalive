#!/usr/bin/env bash
# Despliegue: vault de clave de emisión + Zoom → IVS (sin tocar el correo)
# gestionado desde la plataforma. Uso en Replit:
#   curl -sL https://codeload.github.com/andrestabla/icazalive/tar.gz/refs/heads/feat/aws-ivs-s3 | tar xz -C /tmp && bash /tmp/icazalive-feat-aws-ivs-s3/scripts/replit-deploy-zoom-live-only.sh
set -euo pipefail
...
# (SendGrid ya está configurado en Replit por su agente con la conexión administrada: no se toca.)
